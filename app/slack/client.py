...
class SlackClient:
    ...
    def send_message(
        ...
        return slack's response JSON (already checked ok==True)
        """
        ...
        if reply_broadcast is not None:
            # Slack expects "true"/"false" or 1/0 ; httpx will encode bool fine
        ...
        def list_channels(self,
            ...
            """ 
            List conversations (channels) using conversation.list.
            Return Slack's raw response JSON (ok already checked)
            """
            
            #  Slack 'types' parameters controls which kinds of conversations are returned.
